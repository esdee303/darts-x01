...
function handleThrow(mult, value, btn){
  ...
  const label = value === 0 ? 'MISS' : `${mult === 1 ? 'S' : mult === 2 ? 'D' : 'T'}${value}`;
...
function renderStats(){
  ...
  t.innerHTML = '<tr><th>Speler</th><th>Gemiddelde (3-dart)</th><th>Checkout%</th><th>First Nine Gem.</th><th>Darts</th><th>Punten</th><th>CO: Succes / Pogingen</th></tr>';
  state.players.forEach(p=>{
    ...
    tr.innerHTML = `<td>${p.name}</td><td>${a}</td><td>${co}</td><td>${f9}</td><td>${p.stats.totalDarts}</td><td>${p.stats.totalPoints}</td><td>${p.stats.checkouts} / ${p.stats.checkoutAttempts}</td>`;
    t.appendChild(tr);
  });
}
